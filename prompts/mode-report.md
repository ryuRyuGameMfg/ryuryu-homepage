...
1. ~/repository/homepage-agent/memory/hot/ の直近 7日分を読んでサマリー作成
2. 週次レポートを ~/repository/strategy-room/auto-reports/homepage-daily/{{TODAY_UNDERSCORE}}.md に保存
   （フォーマット: 現状サマリー / 更新履歴 / SEO進捗 / 次サイクルの方針）
3. ~/repository/homepage-agent/HEARTBEAT.md の各チェック項目を実際に確認して更新
4. ~/repository/homepage-agent/MEMORY.md を見直し
   （古い情報を降格、新しい重要情報を昇格、100行以内に維持）
5. ~/repository/homepage-agent/memory/hot/{{TODAY}}.md に実行記録を保存

iteration のインクリメントは homepage-agent.sh が自動で行います（不要）。
